...
async def evaluate_gdpr_policy(policy: UploadFile = File(...)):
    ...
    print(f"[API] Saving uploaded file to {file_path}")
    ...
    print("[API] Initializing RegulationCompare for GDPR")
    ...
    print(f"[API] PDF loaded, extracted text length: {len(text)} characters")
    
    with open("../report/pdfs.txt", "w", encoding="utf-8") as f:
    ...
    print("[API] Initializing LLM report generation")
    try:
        llm_report_generator = LLMReport(
            model_url="http://localhost:11434",
            model_name="llama3.2:3b",
            report_path="../report/final_report.txt"
        )
        print("[API] LLM report generator initialized, calling generate_report()")
        llm_report = llm_report_generator.generate_report()
        print(f"[API] LLM report generated, length: {len(llm_report) if llm_report else 0} characters")
    except Exception as e:
        print(f"[API] Error generating LLM report: {e}")
        import traceback
        traceback.print_exc()
        llm_report = "Error generating LLM report"

    print("[API] API call complete, returning response")
    return {
